...
function [mux, muy, muz, rotMobil, x0] = secondM2SymmConeWeighted(b, B, sumNorm, secM, signal, backg)

%% define a least square problem structure

Ix = secM(1) .* (b.XX) + secM(2) .* (b.YY) + secM(3) .* (b.ZZ) + ...
    secM(4) .* (b.XY) + secM(5) .* (b.XZ) + secM(6) .* (b.YZ);
